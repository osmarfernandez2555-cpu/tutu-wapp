...
app.get('/api/bandeja/:telefono', auth, (req, res) => { const tel = req.params.telefono.replace(/\D/g,''); const limit = Math.min(parseInt(req.query.limit)||500, 1000); db.prepare("UPDATE mensajes SET leido = 1 WHERE telefono = ? AND direccion = 'entrante'").run(tel); res.json({ msgs: db.prepare("SELECT * FROM mensajes WHERE telefono = ? ORDER BY id ASC LIMIT ?").all(tel, limit), contacto: db.prepare("SELECT * FROM contacts WHERE telefono = ?").get(tel) }); });
